...
async function claimForUsers(users) {
  ...
  let r = await Promise.all(users.map((a) => a.claim().then((b) => [a, b])));
  console.log("--- users claimed ---");
  r.sort((a, b) => a[0].id < b[0].id).forEach((a) => {
    a[0].currentA = a[1][0];
    a[0].currentB = a[1][1];
...
class User {
  ...
  async init(initialLamports, stakingMint, initialStaking) {
    ...
    this.stakingPubkey =
      await this.stakingMintObject.createAssociatedTokenAccount(this.pubkey);
    if (initialStaking > 0) {
  ...
  async initializePool(poolKeypair, rewardDuration) {
    ...
    let stakingMintVault = await this.stakingMintObject.createAccount(
      poolSigner
    );
    ...
    await this.program.rpc.initialize(poolNonce, {
      accounts: {
  ...
  async stakeTokens(amount) {
    ...
    let poolSigner = _poolSigner;
    console.log(this.userPubkey, "pubkey");

    await this.program.rpc.stake(new anchor.BN(amount), new anchor.BN(14), {
      accounts: {
        // Stake instance.
        pool: this.poolPubkey,
  ...
  async unstakeTokens(amount) {
    let poolObject = await this.program.account.pool.fetch(this.poolPubkey);
    console.log(poolObject.adminRewardAmount.toNumber(), "pool");
    const [_poolSigner, _nonce] =
      await anchor.web3.PublicKey.findProgramAddress(
        [this.poolPubkey.toBuffer()],
        this.program.programId
      );
    let userObj = await this.program.account.user.fetch(this.userPubkey);
    console.log(userObj, "user");
    let poolSigner = _poolSigner;

    await this.program.rpc.unstake(new anchor.BN(amount), new anchor.BN(7), {
      accounts: {
        // Stake instance.
        pool: this.poolPubkey,
        stakingVault: poolObject.stakingVault,
  ...
  async authorizeFunder(newFunder) {
    await this.program.rpc.authorizeFunder(newFunder, {
      accounts: {
        pool: this.poolPubkey,
        authority: this.provider.wallet.publicKey,
      },
  ...
  async deauthorizeFunder(oldFunder) {
    await this.program.rpc.deauthorizeFunder(oldFunder, {
      accounts: {
        pool: this.poolPubkey,
        authority: this.provider.wallet.publicKey,
      },
  ...
  async fund(amount) {
    ...
    await this.program.rpc.fundStaking(new anchor.BN(amount), {
      accounts: {
        // Stake instance.
        pool: pubkeyToUse,
        stakingVault: poolObject.stakingVault,
        funder: this.provider.wallet.publicKey,
        from: this.stakingPubkey,
        // Program signers.
        poolSigner,
        tokenProgram: TOKEN_PROGRAM_ID,
      },
  ...
  static async getPendingRewardsFunction(
    rewardsPoolAnchorProgram,
    rewardsPoolPubkey
  ) {
    const U64_MAX = new anchor.BN("18446744073709551615", 10);

    const [userPubkey, _userNonce] =
      await anchor.web3.PublicKey.findProgramAddress(
        [
          rewardsPoolAnchorProgram.provider.wallet.publicKey.toBuffer(),
          rewardsPoolPubkey.toBuffer(),
        ],
        rewardsPoolAnchorProgram.programId
      );
    let userObject = await rewardsPoolAnchorProgram.account.user.fetch(
      userPubkey
    );
  ...
  async claim() {
    ...
    await this.program.rpc.claim({
      accounts: {
        ...
        user: this.userPubkey,
        owner: this.provider.wallet.publicKey,
        rewardAAccount: this.stakingPubkey,
        // Program signers.
        poolSigner,
        tokenProgram: TOKEN_PROGRAM_ID,
      },
    ...
    let amtB = await this.provider.connection.getTokenAccountBalance(
      this.stakingPubkey
    );

    return [amtB.value.uiAmount];
  }
